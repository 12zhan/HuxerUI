  ...
  if (show_handler) {
    ...
    application.OnActivation(
        [version](ApplicationActivation activation) {
          ...
          received_activations.push_back(UrlActivation{Uri("test:" + std::to_string(version))});
        },
        version
    );
  }
...
View ReentrantActivationApp() {
  ...
  application.OnActivation([](ApplicationActivation activation) {
    ...
    if (received_activations.size() == 1) {
      active_runtime->HandleApplicationActivation(UrlActivation{Uri("test:second")});
    }
  });
  return {};
}
...
TEST_CASE("Application exposes its immutable startup activation") {
  ...
  TestPlatform platform;
  Runtime runtime(
      ActivationApp, platform, {.show_debug_overlay = false}, UrlActivation{Uri("huxerui://documents/42")}
  );
  runtime.SetWindowMetrics({.viewport = {320.0F, 240.0F}});

  runtime.BuildFrame();

  REQUIRE(std::get<UrlActivation>(startup_activation).url.ToString() == "huxerui://documents/42");
...
TEST_CASE("Application delivers subsequent activations in FIFO order without deduplication") {
  ...
  runtime.HandleApplicationActivation(UrlActivation{Uri("huxerui://same")});
  runtime.HandleApplicationActivation(UrlActivation{Uri("huxerui://same")});
  runtime.HandleApplicationActivation(UrlActivation{Uri("huxerui://last")});
  runtime.BuildFrame();

  REQUIRE(received_activations.size() == 3);
  REQUIRE(std::get<UrlActivation>(received_activations[0]).url.ToString() == "huxerui://same");
  REQUIRE(std::get<UrlActivation>(received_activations[1]).url.ToString() == "huxerui://same");
  REQUIRE(std::get<UrlActivation>(received_activations[2]).url.ToString() == "huxerui://last");
}
...
TEST_CASE("Application retains activation until a composition handler is connected") {
  ...
  runtime.HandleApplicationActivation(UrlActivation{Uri("huxerui://waiting")});
  runtime.BuildFrame();
  REQUIRE(received_activations.empty());
  ...
  REQUIRE(received_activations.empty());
  runtime.BuildFrame();
  REQUIRE(std::get<UrlActivation>(received_activations.front()).url.ToString() == "test:1");

  handler_version = 2;
  runtime.BuildFrame();
  runtime.HandleApplicationActivation(UrlActivation{Uri("huxerui://latest-handler")});
  runtime.BuildFrame();
  REQUIRE(std::get<UrlActivation>(received_activations.back()).url.ToString() == "test:2");

  const std::size_t delivered_count = received_activations.size();
  show_handler = false;
  runtime.BuildFrame();
  runtime.HandleApplicationActivation(UrlActivation{Uri("huxerui://while-disconnected")});
  runtime.BuildFrame();
...
TEST_CASE("Application defers activations enqueued by a handler until the next frame") {
  ...
  runtime.BuildFrame();
  REQUIRE(received_activations.size() == 1);
  REQUIRE(std::get<UrlActivation>(received_activations.front()).url.ToString() == "test:first");

  runtime.BuildFrame();
  REQUIRE(received_activations.size() == 2);
  REQUIRE(std::get<UrlActivation>(received_activations.back()).url.ToString() == "test:second");
}
