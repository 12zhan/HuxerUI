...
| Clipboard | `Clipboard`, `UseService<Clipboard>`, `IsAvailable`, `ReadText`, `WriteText` | [resources-files-network.md](resources-files-network.md#clipboard) | `clipboard.h`, `environment.h` |
| Files and network | `Bytes`, `Uri`, `File`, `FileReference::AsFile`, `PlatformPayload(FileReference)`, `FilePicker::OpenDirectoryAsync`, `CopyDirectoryContentsToAsync`, `DirectoryCopySummary`, `FileSystem`, `HttpClient` | [resources-files-network.md](resources-files-network.md) | `data.h`, `file.h`, `platform_registry.h`, `http.h` |
