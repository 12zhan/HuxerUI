...
// Internal family name recorded in a font file, or empty when it cannot be
// read. Fontconfig indexes an application font file under this internal name,
// while Font values carry the generated registration name.
std::string FontFileFamilyName(const std::string& filename) {
  int count = 0;
  FcPattern* pattern =
      FcFreeTypeQuery(reinterpret_cast<const FcChar8*>(filename.c_str()), 0, nullptr, &count);
  if (pattern == nullptr) {
    return {};
  }
  FcChar8* family_value = nullptr;
  std::string family_name;
  if (FcPatternGetString(pattern, FC_FAMILY, 0, &family_value) == FcResultMatch && family_value != nullptr) {
    family_name = reinterpret_cast<const char*>(family_value);
  }
...
std::string RegisterCustomFont(std::string_view family) {
  ...
  std::string internal_name = FontFileFamilyName(filename);
  if (internal_name.empty()) {
    internal_name = resolved;
  }
  attempted_families[resolved] = internal_name;
...
struct LinuxRenderer::State {
  ...
  SnapshotTexture AcquireSnapshotTexture(const std::shared_ptr<ExternalTexture>& texture) {
    ...
    if (gdk_texture) {
      ...
      return {frame ? GDK_TEXTURE(g_object_ref(frame->Texture())) : nullptr, false};
    }
...
private:
  ...
  void DrawCommand(CairoBatch& batch, const PaintCommand&, const DrawExternalTextureCommand& command) {
    ...
    LinuxRenderer::State::SnapshotTexture acquired = state_.AcquireSnapshotTexture(command.texture);
    ...
    if (acquired.flip_y) {
      gtk_snapshot_save(snapshot_);
      graphene_point_t flip_offset;
      graphene_point_init(&flip_offset, 0.0F, texture_bounds.y * 2.0F + texture_bounds.height);
      gtk_snapshot_translate(snapshot_, &flip_offset);
      gtk_snapshot_scale(snapshot_, 1.0F, -1.0F);
    }
    ...
    gtk_snapshot_pop(snapshot_);
    g_object_unref(acquired.texture);
  }
